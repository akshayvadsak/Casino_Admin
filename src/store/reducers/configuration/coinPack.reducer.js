...
import { createCoinPack, getCoinPack, updateCoinPack, deleteCoinPack, getCoinPackList } from 'store/thunk/configuration/coinPack.thunk';
...
const CoinPackSlice = createSlice({
    ...
    extraReducers: {
        ...
        [getCoinPackList.pending]: (state) => {
            state.status = 'loading';
        },
        [getCoinPackList.fulfilled]: (state, { payload }) => {
            if (payload.status === true) {
                state.msg = payload.msg;
        ...
        [getCoinPackList.rejected]: (state) => {
            state.status = 'failed';
            state.msg = 'Something went wrong. Please try again.';
        },
