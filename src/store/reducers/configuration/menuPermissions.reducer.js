...
const MenuPermissionsSlice = createSlice({
    ...
    extraReducers: {
        ...
        // Refresh permissions Reducers
        [refreshMenuPermissions.pending]: (state) => {
            state.status = 'loading';
        },
        [refreshMenuPermissions.fulfilled]: (state, { payload }) => {
            if (payload.status === true) {
                state.msg = payload.msg;
                state.menuPermissionsList = payload.data;
                state.status = 'success';
            }
            if (payload.status === false) {
                state.msg = payload.msg || 'Network Error';
        ...
        [refreshMenuPermissions.rejected]: (state) => {
            state.status = 'failed';
            state.msg = 'Something went wrong. Please try again.';
        },
        ...
        [updateMenuPermissions.fulfilled]: (state, { payload }) => {
            if (payload.status === true) {
                state.msg = payload.msg;
                state.data[parseInt(state.dataIndex)] = payload.data[0];
                state.status = 'success';
            }
            if (payload.status === false) {
                state.msg = payload.msg;
