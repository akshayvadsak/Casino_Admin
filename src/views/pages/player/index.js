...
function Players() {
    ...
    useEffect(() => {
        const { agentPermissions, menuPermissions } = handlePermissions({ url: window.location.href, isSubMenu: false });
            ...
            options: {
                ...
                customBodyRenderLite: (dataIndex) => (
                    <>
                        ...
                        {agentPermissions.includes('BLOCK_PLAYER') ? (
                            <Tooltip title={players.data[dataIndex].ISACTIVE == true ? 'Block' : 'Unblock'}>
                                <IconButton
                                    color={players.data[dataIndex].ISACTIVE == true ? 'success' : 'error'}
                                    onClick={() => {
                                        setOpenDialog(!openDialog);
                                        dispatch(setDataIndex(dataIndex));
                                        setPlayerId(players.data[dataIndex].PLAYER_ID);
                                        setPlayerStatus(players.data[dataIndex].ISACTIVE);
                                    }}
                                >
                                    <BlockIcon />
                                </IconButton>
            ...
            <Box>
                <Paper>
                    <MainCard
                        ...
                        secondary={
                            agentPermissions.includes('REGISTER_PLAYER') &&
                    ...
                    >
                        {isMobileDevice && agentPermissions.includes('REGISTER_PLAYER') && menuPermissions.ISNEW && (
                            <>
                                <Button
                                    startIcon={<AddIcon />}
                                    fullWidth
                        ...
                        <Box>
                            {players.data.length > 0 && menuPermissions.ISVIEW ? (
