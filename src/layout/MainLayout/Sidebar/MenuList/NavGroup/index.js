...
const NavGroup = ({ item }) => {
    ...
    user.menupermission.map((menupermission) => {
        urlArray.push(menupermission.MENU_SLUG);
    });

    menuItemList.children?.map((menuItem, index) => {
        switch (menuItem.type) {
            case 'item':
                if (!urlArray.includes(menuItem.url)) {
                    menuItemList.children.splice(index, 1);
                }
                return menuItem;
            case 'collapse':
                menuItem.children?.map((collapseMenuItem, idx) => {
                    if (!urlArray.includes(collapseMenuItem.url)) {
